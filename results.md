...
이전 실행에서는 "이번 시즌"의 기준 시점을 모델이 몰라, 방식2가 1년 지난 **2024-25 시즌(SGA)** 으로
잘못 큐레이션하는 문제가 있었습니다. 이를 두 가지로 보완했습니다.

1. **현재 날짜 프롬프트 주입 (두 방식 공통)** — "오늘은 2026-08-25이며 '이번 시즌/최신'은 이 날짜 기준으로 해석하라"를 시스템/입력 프롬프트에 명시.
2. **발행일 필터 (방식2 전용)** — AgentCore Web Search 커넥터의 `publishedDateFilter`로 **최근 30일(2026-07-25 ~ 2026-08-24)** 발행 결과만 검색하도록 제한.

결과적으로 **두 방식 모두 2025-26 시즌(현재 기준 직전 시즌 = 최신)** 으로 정렬되었습니다.
...
특히 방식2는 발행일 필터(최근 30일) 덕분에 웹 검색 결과가 전부 2025-26 시즌을 다룬 최신 기사
(`Picking the NBA moment that defined the 2025-26 season` 등)로 바뀌어, 이전의 2024-25 시즌 혼선이 사라졌습니다.
